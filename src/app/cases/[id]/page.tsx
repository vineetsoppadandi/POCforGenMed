...
export default function StationPage() {
  ...
  const [studyMode, setStudyMode] = useState(true); // study = hints visible
      ...
      try {
        const res = await fetch("/api/chat", {
          ...
          body: JSON.stringify({
            ...
            systemPrompt: osce.patientSystemPrompt,
          }),
        });
        if (res.ok) {
  ...
  function toggleMark(itemId: string, maxMarks: number) {
    setMarkedItems((prev) => {
      const current = prev[itemId] ?? 0;
      const next = current >= maxMarks ? 0 : current + 1;
                    ...
                    {isExpanded && (
                      <div className="border-t border-slate-100 divide-y divide-slate-50">
                        {cat.items.map((item) => {
                          ...
                          const partial = awarded > 0 && awarded < item.marks;
                          const full = awarded >= item.marks;
